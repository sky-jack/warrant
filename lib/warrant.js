(function( $ ) {
	...
	Plugin.prototype.bindPreSubmitValidation = function () {
		var self = this;

		self.$inputs.blur(function () {
			var $input = $(this);
			if (mdown) {
				$(document).bind('mouseup.validation touchend.validation CheckboxStateChange.validation click', function(){
					self.checkElements($input);
					$(document).unbind('mouseup.validation touchend.validation CheckboxStateChange.validation click');
				});
			} else {
				self.checkElements($input);
			}
	...
	Plugin.prototype.init = function () {
		...
		self.bindFormSubmit(self.$element);
		if (self.options.presubmit == true) {
			self.bindPreSubmitValidation();
		};
